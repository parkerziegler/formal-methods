...
def case1(color1, color2, state):
    "Does not change state, so just return."
    return state
...
def case3(color1, color2, state):
    (ycnt, bcnt, rcnt) = state
    cnd = z3.Or(
        z3.And(color1 == BLUE, color2 != RED, color2 != NONE),
        z3.And(color2 == BLUE, color1 != RED, color1 != NONE),
    )
    rcntp = z3.If(cnd, rcnt + 10, rcnt)
...
def relation(p, q):
    ...
    yellow_lt = yellow_q < yellow_p
    blue_lt = z3.And(yellow_q == yellow_p, blue_q < blue_p)
    red_lt = z3.And(yellow_q == yellow_p, blue_q == blue_p, red_q < red_p)

    return z3.Or(yellow_lt, blue_lt, red_lt)
