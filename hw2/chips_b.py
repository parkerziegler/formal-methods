...
def is_total_relation():
    ...
    state2 = z3.Int("ycnt2"), z3.Int("bcnt2"), z3.Int("rcnt2")
    eq = z3.And(
        (state1[0] == state2[0]), (state1[1] == state2[1]), (state1[2] == state2[2])
    )
...
def case1(color1, color2, state):
    (ycnt, bcnt, rcnt) = state
    cnd = z3.Or(
        z3.And(color1 == RED, color2 == YELLOW), z3.And(color1 == YELLOW, color2 == RED)
    )

    # Replace the red chip in the bag.
...
def case3(color1, color2, state):
    (ycnt, bcnt, rcnt) = state
    cnd = z3.Or(
        z3.And(color1 == BLUE, color2 == RED), z3.And(color1 == RED, color2 == BLUE)
    )

    # Add 10 blue chips to the bag.
    bcntp = z3.If(cnd, bcnt + 10, bcnt)
    return (ycnt, bcntp, rcnt)


def relation(p, q):
    ((yellow_p, blue_p, red_p), (yellow_q, blue_q, red_q)) = (p, q)

    red_lt = red_q < red_p
    yellow_lt = z3.And(red_q == red_p, yellow_q < yellow_p)
    blue_lt = z3.And(red_q == red_p, yellow_q == yellow_p, blue_q < blue_p)

    return z3.Or(red_lt, yellow_lt, blue_lt)
